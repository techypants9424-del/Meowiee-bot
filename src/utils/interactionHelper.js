...
export class InteractionHelper {
    ...
    static async safeDefer(interaction, options = {}) {
        try {
            ...
            if (
                error.name === 'InteractionAlreadyReplied' ||
                error.code === 40060
            ) {
                logger.warn(
    ...
    // Used for buttons and select-menu interactions
    static async safeUpdate(interaction, options = {}) {
        try {
            if (!this.isInteractionValid(interaction)) {
                logger.warn(
                    `Interaction ${interaction.id} has expired before update, ignoring`
                );
            ...
            if (interaction.replied || interaction.deferred) {
                logger.warn(
                    `Interaction ${interaction.id} already acknowledged, cannot update`
                );
            ...
            if (isInteractionUnavailableError(error)) {
                logger.warn(
                    `Interaction ${interaction.id} unavailable during update:`,
                    error.message
                );
            ...
            if (error.code === 40060) {
                logger.warn(
                    `Interaction ${interaction.id} already acknowledged during update:`,
                    error.message
                );
    ...
    static async safeExecute(
        ...
        if (
            ...
            const deferSuccess = await this.safeDefer(
                interaction,
                deferOptions
            );
    ...
    static async universalReply(interaction, options) {
        ...
        if (interaction._isPrefixCommand) {
            ...
            return (
                (await coordinator?.respond(options)) ??
                this.safeReply(interaction, options)
            );
        }
